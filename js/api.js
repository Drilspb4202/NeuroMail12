...
class MailSlurpApi {
    ...
    async getEmail(inboxId, emailId) {
        try {
            ...
            // Помечаем письмо как прочитанное, если предоставлен ID ящика
            if (inboxId && inboxId !== emailId) {
                this.markEmailAsRead(emailId, inboxId)
                    .then(() => console.log('Письмо помечено как прочитанное'))
                    .catch(err => console.warn('Ошибка при отметке письма как прочитанного:', err));
            }
