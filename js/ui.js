...
class MailSlurpUI {
    ...
    /**
     * Показать индикатор загрузки содержимого письма
     */
    ...
    showEmailViewer(email) {
        // Сначала скрываем для плавной анимации
        this.emailViewer.style.display = 'block';
        setTimeout(() => {
            this.emailViewer.classList.add('active');
        }, 10);
        
        // Информация о письме
        this.emailFrom.textContent = email.from || 'Unknown';
        this.emailTo.textContent = email.to.join(', ') || email.to;
        this.emailSubject.textContent = email.subject || '(No subject)';
        
        // Форматирование даты
        let date = new Date(email.createdAt);
        this.emailDate.textContent = date.toLocaleString();
        
        // Очищаем тело письма
        this.emailBody.innerHTML = '';
        
        // Показываем прогресс загрузки
        this.showEmailContentLoading();
        
        // Обработка содержимого письма в зависимости от формата
        if (email.body) {
            // Определяем, HTML это или обычный текст
            if (email.body.trim().startsWith('<')) {
                // Создаем iframe для изолированного отображения HTML
                const iframe = document.createElement('iframe');
                iframe.className = 'email-content-iframe';
                iframe.sandbox = 'allow-same-origin allow-popups allow-popups-to-escape-sandbox'; // Безопасный режим
                ...
                iframe.onload = () => {
                    // Скрываем загрузчик
                    const loader = this.emailBody.querySelector('.email-content-loader');
                    if (loader) {
                ...
                // Создаем HTML-содержимое с базовой структурой
                iframeDocument.open();
                ...
                // Автоматически подстраиваем высоту iframe под содержимое
                const resizeIframe = () => {
                    try {
                        ...
                        console.warn('Ошибка при изменении размера iframe:', e);
                    }
                };
                
                // Вызываем изменение размера при загрузке и создаем MutationObserver
                if (iframe.contentWindow) {
                    iframe.contentWindow.onload = resizeIframe;
                    
                    try {
                        // Создаем MutationObserver для отслеживания изменений в содержимом
                        const observer = new MutationObserver(resizeIframe);
                        
                        // Начинаем отслеживать изменения в DOM iframe
                        ...
                        console.warn('Ошибка при создании MutationObserver:', e);
                    }
                }
                
                // Запускаем проверку размера несколько раз, чтобы учесть загрузку изображений
                setTimeout(resizeIframe, 100);
                setTimeout(resizeIframe, 500);
                setTimeout(resizeIframe, 1000);
                setTimeout(resizeIframe, 2000);
            } else {
                // Простой текст - отображаем в pre-элементе для сохранения форматирования
                const preElement = document.createElement('pre');
                preElement.className = 'email-content-text';
                preElement.textContent = email.body;
                this.emailBody.appendChild(preElement);
            ...
            // Если содержимое не загружено
            const errorElement = document.createElement('div');
            errorElement.className = 'email-content-error';
            errorElement.textContent = 'Содержимое письма не может быть загружено';
            this.emailBody.appendChild(errorElement);
